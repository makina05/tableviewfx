...
public class Backup {
    ...
    public void loadProps() {
        InputStream in = this.getClass().getResourceAsStream("/setup.properties");
        try {
            configProp.load(in);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public Backup(File authsDir) throws FlickrException {

        loadProps();
        String apiKey = configProp.getProperty("apiKey");
        String  nsid = configProp.getProperty("nsid");
        String sharedSecret = configProp.getProperty("sharedSecret");
    ...
    public void doBackup() throws Exception {
        File directory = new File(configProp.getProperty("output_dir"));
        if (!directory.exists()) {
            directory.mkdir();
        }
        ...
        int i = 1;
        while (sets.hasNext() && i < 10) {
            Photoset set = (Photoset) sets.next();
            PhotoList photos = pi.getPhotos(set.getId(), 10, 1);
...
//
        ...
        Backup bf = new Backup(new File(System.getProperty("user.home") + File.separatorChar + ".flickrAuth"));
        bf.doBackup();
    }
}