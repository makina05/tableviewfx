...
public class WhatWebDBKud {
    ...
    public List<Laguntzailea> lortuOrrialdeak(){
        String query = "select t.target, s.version, s.string, t.target_id from targets t inner join scans s on t.target_id=s.target_id";
        ...
        try{
            while (rs.next()){
                ...
                Laguntzailea laguntzailea = new Laguntzailea(target,version,lastUp,cms);
